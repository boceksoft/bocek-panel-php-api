...
IF OBJECT_ID(N'[dbo].[dolu]', N'U') IS NOT NULL
    AND COL_LENGTH(N'[dbo].[dolu]', N'IsExternal') IS NULL
BEGIN
ALTER TABLE [dbo].[dolu] ADD [IsExternal] bit NULL CONSTRAINT DF_dolu_IsExternal DEFAULT 0;
END;
GO
...
IF OBJECT_ID(N'[dbo].[GuestWelcomeMessages]', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[GuestWelcomeMessages]
(
    [rezid] int NOT NULL,
    [HosgeldinMesaji] nvarchar(MAX) NOT NULL
    CONSTRAINT [DF_GuestWelcomeMessages_HosgeldinMesaji] DEFAULT N'Hoşgeldin Mesajı'
    );
END;
GO

IF OBJECT_ID(N'[dbo].[GuestWelcomeMessages]', N'U') IS NOT NULL
    AND COL_LENGTH(N'[dbo].[GuestWelcomeMessages]', N'rezid') IS NULL
BEGIN
ALTER TABLE [dbo].[GuestWelcomeMessages] ADD [rezid] int NULL;
END;
GO

IF OBJECT_ID(N'[dbo].[GuestWelcomeMessages]', N'U') IS NOT NULL
    AND COL_LENGTH(N'[dbo].[GuestWelcomeMessages]', N'HosgeldinMesaji') IS NULL
BEGIN
ALTER TABLE [dbo].[GuestWelcomeMessages] ADD [HosgeldinMesaji] nvarchar(MAX) NOT NULL
    CONSTRAINT [DF_GuestWelcomeMessages_HosgeldinMesaji_Add] DEFAULT N'Hoşgeldin Mesajı';
END;
GO
...
IF OBJECT_ID(N'[dbo].[GuestWelcomeMessages]', N'U') IS NOT NULL
    AND NOT EXISTS (
        SELECT 1
        FROM sys.indexes
        WHERE name = N'UX_GuestWelcomeMessages_rezid'
          AND object_id = OBJECT_ID(N'[dbo].[GuestWelcomeMessages]')
    )
BEGIN
CREATE UNIQUE INDEX [UX_GuestWelcomeMessages_rezid]
    ON [dbo].[GuestWelcomeMessages] ([rezid])
    WHERE [rezid] IS NOT NULL;
END;
GO
